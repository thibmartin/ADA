...
Despite having been fine-tuned on a dataset with coming from a source different than media articles, we can clearly see that the sentiment distribution is much more balanced than VADER while being more polarised towards negative and positive than neutral. After also having looked at the labels assigned to a sample of quotes, we assume that it is a satisfying result for the purpose of our analysis.

We also transformed the quotes into 768-dimensional embedding vectors with a pre-trained BERT expert model trained on Wikipedia and book corpus, and fine-tuned on SST-2 dataset in order to be employed for sentiment analysis tasks. We applied principal component analysis to the vector embeddings to reduce the dimensionality to 2.
...
Aggression is what appears the most. This might be either a consequence of people speaking aggressively about gun violence or because our chosen theme is inherently violent and matches with a lot of keywords that might be considered as "aggressive" or both to a different degree. Furthermore, we notice a slight increasing trend in aggression over the entire dataset.
We also notice that speakers regularly express fear and sadness. Funnily enough, there seems to be a big peak in those two emotions in the time frame following the 2016 presidential election. We also note optimism and finally, we notice speakers usually display little anger, disappointment and joy.  

#### Shootings event and number of quotes correlation analysis

We did a selection of important shooting events first by visualising which events triggered a noticeable spike in number of quotes. This appears to concern events with at least 7 deaths apart from a few exception that we choose to drop as we consider having a satisfying number of event to analyse. We are left with the following 15 events:
...
Despite high variance and some blank periods we can nonetheless extract some information from these graphs.

Barack Obama is often sad when talking about gun violence. 

Hillary Clinton is aggressively active on the gun violence topic prior to the 2016 election, but suddenly falls silent following her loss. Perhaps she was using this highly politicked topic as a tool to gain following among the community that supports wider gun control?
...
Alan Gottlieb, a conservative political activist and gun rights advocate also regularly talks about gun violence. He seems to do so expressing regular suffering, fear, aggression, but also optimism ... This last emotion is not displayed as often be speakers who would favour gun control.
...
We observed a correlation between a shooting event and the number of quotes, but as we know, correlation doesn't imply causation. Even though we tried to reduce the noise as much as possible, there are still too many hidden cofactors that could bias our analysis. Outside events can have a lot of impact on the number of quotes, either directly (in the case of the gun violence rally in early 2018 for example) or indirectly, because if another significant event generates a lot of quotes, we will have less focus on the gun violence topic. 

Nevertheless, we identified a significant increase in the number of quotes related to gun violence, a significant shift in sentiment towards negative values as well as a spike in negative emotions (i.e. aggression, sadness and fear) in correspondence to relevant gunshot violence events. Furthermore, we showed that sentiment distribution seem to quickly shifts back to the "norm" in just a matter of days after the events.

We were not able to get satisfying results for the sentiment evolution of only the quotes assigned to particular speakers part of the political sphere but noticed significant figures display varying types of emotion with respect to gun violence. Alan Gottlieb, a gun activist, displayed more hope than others, while Barack Obama showed more sadness. We've also been given hints that gun violence is used as a political tool by presidential candidates, such as Bernie Sanders and Hillary Clinton. 
<p align="center">
